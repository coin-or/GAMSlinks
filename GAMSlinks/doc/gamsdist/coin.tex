...
NLPs are solved in \BONMIN by \IPOPT, which uses \textsc{MUMPS}~\cite{AmestoyDuffKosterLExcellent2001,AmestoyGuermoucheLExcellentPralet2006} as linear solver.
...
\textsc{MUMPS} and \textsc{MA57} use \textsc{METIS} for matrix ordering \cite{KaKu99}, cf.~\url{http://glaros.dtc.umn.edu/gkhome/views/metis/index.html} and \url{http://glaros.dtc.umn.edu/gkhome/fetch/sw/metis/manual.pdf}.
...
However, the expected behavior that \IPOPT finishes within one iteration if optimal primal and dual values are passed is not reached this way yet. This is, because \IPOPT by default moves any initial value that is close to a bounds into the interior. The amount on how much the initial point is moved can be controlled by various \texttt{bound\_push} and \texttt{bound\_frac} options.
To allow \IPOPT accept an optimal primal/dual solution within one iteration, it should be sufficient to set the following options:
\begin{verbatim}
