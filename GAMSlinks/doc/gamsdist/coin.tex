...
With the availability of source code for the GAMS/COIN-OR links the user is not limited to the out of the box GAMS/COIN-OR solvers that come with a regular GAMS distribution.
...
This can be set by using the option named \texttt{bonmin.algorithm} which can be set to \texttt{B-BB}, \texttt{B-OA}, \texttt{B-QG}, \texttt{B-Hyb}, or \texttt{B-Ecp} (its default value is \texttt{B-Hyb}).
Depending on the value of this option, certain other options may be available or not.

In the context of outer approximation decomposition, several options are available for configuring the MILP subsolver CBC.
...
the number of strong-branching candidates,
the number of branches before pseudo costs are to be trusted, and the frequency of some cut generators.
...
\begin{verbatim}
   ...
   print_level 6
\end{verbatim}
This sets the algorithm to be used to the hybrid algorithm, the level of outer approximation related output to $4$, the print level for \texttt{Ipopt} to $6$, the MILP subsolver for outer approximation to a parametrized version of CBC, and switches off cover cutting planes for the MILP subsolver.
...
The iteration log will have an entry for every 20th iteration instead of every 10th iteration.
...
\begin{verbatim}
  scaling mean
\end{verbatim}
It will cause CoinGlpk to change the scaling method to a geometric mean scaling.
