...
Specifying \texttt{optfile=1} let \GAMS/\BONMIN read \texttt{coinbonmin.opt}, \texttt{optfile=2} corresponds to \texttt{coinbonmin.op2}, and so on.
...
default settings, {\tt Cbc\_Par} which uses a version of \CBC that can be parameterized by the user, and
\texttt{Cplex} which uses \CPLEX with its default settings.
...
The following tables gives the list of options together with their types and default values.
The column labeled `type' indicates the type of the parameter (`F' stands for float, `I' for integer, and `S' for
string).
The column labeled `default' indicates the default value.
...
\GAMS/\IPOPT includes the sparse solver MUMPS 4.9 (\url{http://graal.ens-lyon.fr/MUMPS}).
