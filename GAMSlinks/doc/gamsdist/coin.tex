...
\end{verbatim}
An small example for a coinscip.opt file is:
\begin{verbatim}
  ...
  gams/mipstart = FALSE
\end{verbatim}
It will cause SCIP to turn off all cut generators.
Further, GAMS will give row and column names to SCIP which are then used for debugging output,
the final solve of the MIP with fixed discrete variables to obtain dual values is skipped, and it does not try to use the initial column levels as a first solution point.
