...
% \MYGAMS/\BONMIN brings the open source MINLP solver \BONMIN to the broad audience of \MYGAMS users.
...
For more information we refer to \cite{BoCoLoMa06,BoKiLi09,BoGo08,BBCCGLLLMSW} and the \BONMIN web site \texttt{https://projects.coin-or.org/Bonmin}.
Most of the \BONMIN documentation in this section is taken from the \BONMIN manual~\cite{BonminManual}.

% \MYGAMS/BonminD is an experimental in-core communication link.
% It offers in-core communication between \MYGAMS and the solver, making potentially large model scratch files unnecessary.
% This links supports all features of the traditional link except for the \MYGAMS branch-cut-heuristic facility (BCH).
...
\begin{verbatim}
  Option MINLP = BONMIN;     { or LP, RMIP, MIP, DNLP, NLP, RMINLP, QCP, RMIQCP, MIQCP }
\end{verbatim}
...
\MYGAMS/\BONMIN currently does not support the \MYGAMS Branch-and-Cut-and-Heuristic (BCH) Facility.
...
% \MYGAMS/\BONMIN supports the \MYGAMS Branch-and-Cut-and-Heuristic (BCH) Facility.
% The \MYGAMS BCH facility automates all major steps necessary to define, execute, and control the use of user defined routines within the framework of general purpose MIP and MINLP codes.
...
Specifying \texttt{optfile=1} let \MYGAMS/\BONMIN read \texttt{coinbonmin.opt}, \texttt{optfile=2} corresponds to \texttt{coinbonmin.op2}, and so on.
...
\MYGAMS/\BONMIN understands currently the following \MYGAMS parameters: \texttt{reslim} (time limit), \texttt{iterlim} (iteration limit), \texttt{nodlim} (node limit), \texttt{cutoff}, \texttt{optca} (absolute gap tolerance), and \texttt{optcr} (relative gap tolerance).
...
For example, to run a maximum of 60 seconds of FP for MINLP until 6 solutions are found at the beginning of the hybrid algorithm, one sets the following options:
\begin{verbatim}
bonmin.algorithm              B-Hyb
...
% , and \texttt{Cplex} which uses \CPLEX with its default settings.
...
% The \texttt{Cplex} is currently not available on Macintosh platforms.
% Further, a valid \CPLEX licence (standalone or \MYGAMS/\CPLEX) is required.
...
To use \IPOPT more efficiently in the context of the solution of a MINLP problems,
...
\item {\tt print\_level} is set to $0$ by default to turn off \IPOPT output.
...
The column labaled `Cbc\_Par' indicates the options that can be used to parametrize the MIP subsolver in the context of OA and FP.
...
% \MYGAMS/\CBC brings the open source LP/MIP solver \CBC to the broad audience of \MYGAMS users.
...
\item the \textsc{Cgl} web site \texttt{https://projects.coin-or.org/Cgl}, and
\item the \textsc{CLP} web site \texttt{https://projects.coin-or.org/Clp}.
\end{itemize}
...
% \MYGAMS/\CBC supports the \MYGAMS Branch-and-Cut-and-Heuristic (BCH) Facility.
...
% \MYGAMS/\COUENNE brings the open source MINLP solver \COUENNE to the broad audience of \MYGAMS users.
...
\MYGAMS/\COUENNE understands currently the following \MYGAMS parameters: \texttt{reslim} (time limit), \texttt{nodlim} (node limit), \texttt{cutoff}, \texttt{optca} (absolute gap tolerance), and \texttt{optcr} (relative gap tolerance).
One can set them either on the command line, e.g. \verb+nodlim=1000+, or inside your \MYGAMS program, e.g. \verb+Option nodlim=1000;+.
...
% \MYGAMS/\IPOPT brings the open source NLP solver \IPOPT to the broad audience of \MYGAMS users.
...
\MYGAMS/\IPOPT includes the sparse solver MUMPS 4.9 (\url{http://graal.ens-lyon.fr/MUMPS}).
...
% Note, that \MYGAMS/\IPOPT overwrites the \IPOPT default setting for the parameters \texttt{bound\_relax\_factor} (set to $10^{-10}$) and \texttt{mu\_strategy} (set to \texttt{adaptive}).
