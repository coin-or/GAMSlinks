...
For more information on the algorithm we refer to~\cite{Waechter2002,WaBi2006} and the \IPOPT web site \url{https://projects.coin-or.org/Ipopt}.
...
In a commerically licensed \GAMS/\IPOPT version, also the linear solvers \textsc{MA27}, \textsc{MA57}, and \textsc{MA86} from the Harwell Subroutines Library (HSL) are available.
...
In a commerically licensed \GAMS/\IPOPT version, also the Harwell Subroutine Library (HSL) solvers MA27, MA57, and MA86 are available.
However, the user can also provide the 
Parallel Sparse Direct Solver PARDISO or 
routines from HSL as shared (or dynamic) libraries to replace MUMPS.
...
The HSL routines are available at \url{http://www.hsl.rl.ac.uk}.
