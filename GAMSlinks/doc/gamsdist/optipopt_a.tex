...
\setlength{\topsep}{0pt}}
\item[\textit{kkt-error}] nonmonotone decrease of kkt-error
\item[\textit{obj-constr-filter}] 2-dim filter for objective and constraint violation
\item[\textit{never-monotone-mode}] disables globalization
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{1-norm}] use the 1-norm (abs sum)
\item[\textit{2-norm-squared}] use the 2-norm squared (sum of squares)
\item[\textit{max-norm}] use the infinity norm (max)
\item[\textit{2-norm}] use 2-norm
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] don't restore accepted iterate
\item[\textit{yes}] restore accepted iterate
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{probing}] Mehrotra's probing heuristic
\item[\textit{loqo}] LOQO's centrality rule
\item[\textit{quality-function}] minimize a quality function
\item[\textit{average\_compl}] base on current average complementarity
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Take at least one iteration per barrier problem
\item[\textit{yes}] Allow fast decrease of mu if barrier test it met
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{monotone}] use the monotone (Fiacco-McCormick) strategy
\item[\textit{adaptive}] use the adaptive update strategy
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] no balancing term is added
\item[\textit{cubic}] Max(0,Max(dual\_inf,primal\_inf)-compl)\^3
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] no penalty term is added
\item[\textit{log}] complementarity * the log of the centrality measure
\item[\textit{reciprocal}] complementarity * the reciprocal of the centrality measure
\item[\textit{cubed-reciprocal}] complementarity * the reciprocal of the centrality measure cubed
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{exact}] Use second derivatives provided by the NLP.
\item[\textit{limited-memory}] Perform a limited-memory quasi-Newton approximation
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{nonlinear-variables}] only in space of nonlinear variables.
\item[\textit{all-variables}] in space of all variables (without slacks)
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{scalar1}] sigma = s\^Ty/s\^Ts
\item[\textit{scalar2}] sigma = y\^Ty/s\^Ty
\item[\textit{constant}] sigma = limited\_memory\_init\_val
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{bfgs}] BFGS update (with skipping)
\item[\textit{sr1}] SR1 (not working well)
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{constant}] set all bound multipliers to the value of bound\_mult\_init\_val
\item[\textit{mu-based}] initialize to mu\_init/x\_slack
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] use bound\_mult\_init\_val and least-square equality constraint multipliers
\item[\textit{yes}] overwrite user-provided point with least-square estimates
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] take user-provided point
\item[\textit{yes}] overwrite user-provided point with least-square estimates
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] don't arbitrarily accept the full step
\item[\textit{yes}] always accept the full step
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{primal}] use primal step size
\item[\textit{bound-mult}] use step size for the bound multipliers (good for LPs)
\item[\textit{min}] use the min of primal and bound multipliers
\item[\textit{max}] use the max of primal and bound multipliers
\item[\textit{full}] take a full step of size one
\item[\textit{min-dual-infeas}] choose step size minimizing new dual infeasibility
\item[\textit{safer-min-dual-infeas}] like "min\_dual\_infeas", but safeguarded by "min" and "max"
\item[\textit{primal-and-full}] use the primal step size, and full step if delta\_x $<$= alpha\_for\_y\_tol
\item[\textit{dual-and-full}] use the dual step size, and full step if delta\_x $<$= alpha\_for\_y\_tol
\item[\textit{acceptor}] Call LSAcceptor to get step size for y
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{1-norm}] use the 1-norm
\item[\textit{2-norm}] use the 2-norm
\item[\textit{max-norm}] use the infinity norm
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] no corrector
\item[\textit{affine}] corrector step towards mu=0
\item[\textit{primal-dual}] corrector step towards current mu
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] use the Newton step to update the multipliers
\item[\textit{yes}] use least-square multiplier estimates
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Always scale the linear system.
\item[\textit{yes}] Start using linear system scaling if solutions seem not good.
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{ma27}] use the Harwell routine MA27
\item[\textit{ma57}] use the Harwell routine MA57
\item[\textit{pardiso}] use the Pardiso package
\item[\textit{mumps}] use MUMPS package
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] no scaling will be performed
\item[\textit{mc19}] use the Harwell routine MC19
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Don't have MA27 solve singular systems
\item[\textit{yes}] Have MA27 solve singular systems
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] check inertia
\item[\textit{yes}] skip inertia check
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Don't check (faster).
\item[\textit{yes}] Check Jacobians and Hessian for Nan and Inf.
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] only look at gradients
\item[\textit{yes}] also consider right hand side
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] don't check; no extra work at beginning
\item[\textit{mumps}] use MUMPS
\item[\textit{wsmp}] use WSMP
\item[\textit{ma28}] use MA28
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{make\_parameter}] Remove fixed variable from optimization variables
\item[\textit{make\_constraint}] Add equality constraints fixing variables
\item[\textit{relax\_bounds}] Relax fixing bound constraints
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Leave final point unchanged
\item[\textit{yes}] Project final point back into original bounds
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Don't assume that all equality constraints are linear
\item[\textit{yes}] Assume that equality constraints Jacobian are constant
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Don't assume that all inequality constraints are linear
\item[\textit{yes}] Assume that equality constraints Jacobian are constant
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{none}] no problem scaling will be performed
\item[\textit{user-scaling}] scaling parameters will come from the user
\item[\textit{gradient-based}] scale the problem so the maximum gradient at the starting point is scaling\_max\_gradient
\item[\textit{equilibration-based}] scale the problem so that first derivatives are of order 1 at random points (only available with MC19)
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] don't print string
\item[\textit{yes}] print string at end of each iteration output
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] don't print statistics
\item[\textit{yes}] print all timing statistics
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] leave bounds on variables
\item[\textit{yes}] replace variable bounds by inequality constraints
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{complete}] Match complete (IPAR(13)=1)
\item[\textit{complete+2x2}] Match complete+2x2 (IPAR(13)=2)
\item[\textit{constraints}] Match constraints (IPAR(13)=3)
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Always redo symbolic factorization when elements were perturbed
\item[\textit{yes}] Only redo symbolic factorization when elements were perturbed if also the inertia was wrong
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Don't assume that matrix is singular if elements were perturbed after recent symbolic factorization
\item[\textit{yes}] Assume that matrix is singular if elements were perturbed after recent symbolic factorization
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] skip evaluation
\item[\textit{yes}] evaluate at every trial point
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] the problem probably be feasible
\item[\textit{yes}] the problem has a good chance to be infeasible
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] don't force start in restoration phase
\item[\textit{yes}] force start in restoration phase
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Verify solution of linear system by computing residuals.
\item[\textit{yes}] Trust that linear systems are solved well.
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Do the usual Ipopt algorithm.
\item[\textit{yes}] Do Mehrotra's predictor-corrector algorithm.
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] perturbation only used when required
\item[\textit{yes}] always use perturbation
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] call GetStartingPoint in the NLP
\item[\textit{yes}] call GetWarmStartIterate in the NLP
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] do not use the warm start initialization
\item[\textit{yes}] use the warm start initialization
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{no}] Assume this is a new problem.
\item[\textit{yes}] Assume this is problem has known structure
\end{list}
}
