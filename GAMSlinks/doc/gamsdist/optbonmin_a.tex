...
\printoption{random\_generator\_seed}%
{$-1\leq\textrm{integer}$}%
{$0$}%
...
\item[\texttt{probed-dive}] Dive in the tree exploring two children before continuing the dive at each level.
...
\printoptioncategory{MILP cutting planes in hybrid algorithm}
...
See option \texttt{2mir\_cuts} for a detailed description.}%
{}
...
Will continue exploring a branch of the tree until "max\_consecutive\_infeasible"consecutive problems are locally infeasible by the NLP sub-solver.}%
{}
...
\printoptioncategory{Output and Loglevel}
...
\printoption{solvetracetimefreq}%
{$0\leq\textrm{real}$}%
{$5$}%
...
\printoptioncategory{Primal Heuristics}
