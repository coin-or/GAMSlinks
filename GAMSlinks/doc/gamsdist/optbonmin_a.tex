...
\setlength{\topsep}{0pt}}
\item[\textit{B-BB}] simple branch-and-bound algorithm,
\item[\textit{B-OA}] OA Decomposition algorithm,
\item[\textit{B-QG}] Quesada and Grossmann branch-and-cut algorithm,
\item[\textit{B-Hyb}] hybrid outer approximation based branch-and-cut,
\item[\textit{B-Ecp}] ecp cuts based branch-and-cut a la FilMINT.
\item[\textit{B-iFP}] Iterated Feasibility Pump for MINLP.
\end{list}
}
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{stop}] Stop when failure happens.
\item[\textit{fathom}] Continue when failure happens.
\end{list}
}

\printoption{node\_comparison}%
{best-bound, depth-first, breadth-first, dynamic, best-guess}%
{best-bound}%
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{best-bound}] choose node with the smallest bound,
\item[\textit{depth-first}] Perform depth first search,
\item[\textit{breadth-first}] Perform breadth first search,
\item[\textit{dynamic}] Cbc dynamic strategy (starts with a depth first search and turn to best bound after 3 integer feasible solutions have been found).
\item[\textit{best-guess}] choose node with smallest guessed integer solution
\end{list}
}
...
{top-node, dive, probed-dive, dfs-dive, dfs-dive-dynamic}%
{probed-dive}%
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{top-node}]  Always pick the top node as sorted by the node comparison function
\item[\textit{dive}] Dive in the tree if possible, otherwise pick top node as sorted by the tree comparison function.
\item[\textit{probed-dive}] Dive in the tree exploring two childs before continuing the dive at each level.
\item[\textit{dfs-dive}] Dive in the tree if possible doing a depth first search. Backtrack on leaves or when a prescribed depth is attained or when estimate of best possible integer feasible solution in subtree is worst than cutoff. Once a prescribed limit of backtracks is attained pick top node as sorted by the tree comparison function
\item[\textit{dfs-dive-dynamic}] Same as dfs-dive but once enough solution are found switch to best-bound and if too many nodes switch to depth-first.
\end{list}
}

\printoption{variable\_selection}%
{most-fractional, strong-branching, reliability-branching, curvature-estimator, qp-strong-branching, lp-strong-branching, nlp-strong-branching, osi-simple, osi-strong, random}%
{strong-branching}%
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{most-fractional}] Choose most fractional variable
\item[\textit{strong-branching}] Perform strong branching
\item[\textit{reliability-branching}] Use reliability branching
\item[\textit{curvature-estimator}] Use curvature estimation to select branching variable
\item[\textit{qp-strong-branching}] Perform strong branching with QP approximation
\item[\textit{lp-strong-branching}] Perform strong branching with LP approximation
\item[\textit{nlp-strong-branching}] Perform strong branching with NLP approximation
\item[\textit{osi-simple}] Osi method to do simple branching
\item[\textit{osi-strong}] Osi method to do strong branching
\item[\textit{random}] Method to choose branching variable randomly
\end{list}
}
...
{best-ps-cost, worst-ps-cost, most-fractional, least-fractional}%
{best-ps-cost}%
...
\setlength{\itemindent}{0ex}
\setlength{\topsep}{0pt}}
\item[\textit{best-ps-cost}] Sort by decreasing pseudo-cost
\item[\textit{worst-ps-cost}] Sort by increasing pseudo-cost
\item[\textit{most-fractional}] Sort by decreasing integer infeasibility
\item[\textit{least-fractional}] Sort by increasing integer infeasibility
\end{list}
}
