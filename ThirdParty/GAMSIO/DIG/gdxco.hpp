...
public:
  ...
  {  int sidim = gdxCurrentDim(gdxHandle_); for (int i=0; i<sidim; i++) DP0[i]=AFiltElements[i].c_str();int rc=gdxDataReadSlice (gdxHandle_, DP0, &ADim, DP); return rc; }
  ...
  { int rc=gdxDataReadStr (gdxHandle_, DPout, AVals, &AFDim); int sidim = gdxCurrentDim(gdxHandle_);  for (int i=0; i<sidim; i++) AStrElements[i]=DPout[i]; return rc; }
  ...
  { int rc=gdxDataSliceUELS (gdxHandle_, AIndx, DPout); int sidim = gdxCurrentDim(gdxHandle_);  for (int i=0; i<sidim; i++) AUELs[i]=DPout[i]; return rc; }
  ...
  {  int sidim = gdxCurrentDim(gdxHandle_); for (int i=0; i<sidim; i++) DP0[i]=AStrElements[i].c_str();int rc=gdxDataWriteStr (gdxHandle_, DP0, AVals); return rc; }
