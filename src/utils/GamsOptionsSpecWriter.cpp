...
void GamsOptions::finalize()
{
   ...
   // if no print positions were given (all = 0), then use alphabetic order
   if( groups_by_printpos.count(0) == groups_by_printpos.size() )
   {
      groups_by_printpos.clear();
      int printpos = 0;
      for( auto& g : groups )
         groups_by_printpos.emplace(printpos++, g.second);
   }

   // give each group an index
   int index = 1;
   for( auto& g : groups_by_printpos )
...
void GamsOptions::writeDef()
{
   ...
   if( havedeprecatedsynonym )
   {
      ...
      for( auto& opt : options )
         for( const std::pair<const std::string, bool>& element : opt.synonyms )
            if( element.second )
...
static
void writeDoxygenTableEntry(
   std::ofstream&     f,
   const std::string& solver,
   const GamsOption&  opt,
   bool               shortdoc
   )
{
   f << "| ";
   ...
   )
{
   ...
   for( auto& group : groups_by_printpos )
   {
      ...
      for( auto& opt : options )
      {
         ...
         writeDoxygenTableEntry(f, solver, opt, shortdoc);
      }

      if( hasadvanced )
      {
         f << "| Options for expert users |||" << std::endl;
